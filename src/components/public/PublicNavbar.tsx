    ...
    <nav className="gradient-navy sticky top-0 z-50 shadow-lg">
      <div className="container mx-auto flex items-center justify-between py-3 px-4">
        <Link to="/" className="flex items-center gap-3">
          <img src={logo} alt="Winter Knights" className="h-14 drop-shadow-[0_0_8px_hsl(var(--gold)/0.5)]" />
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link
            to="/companies/login"
            className="ml-4 rounded-md gradient-gold px-5 py-2 font-display text-sm uppercase tracking-wider text-primary transition hover:opacity-90"
          >
            Login / Register
          </Link>
        </div>
      ...
      {open && (
        <div className="md:hidden gradient-navy border-t border-gold/20 px-4 pb-4 space-y-3">
          ...
          <Link
            to="/companies/login"
            onClick={() => setOpen(false)}
            className="block rounded-md gradient-gold px-5 py-2 font-display text-sm uppercase tracking-wider text-primary text-center"
          >
            Login / Register
          </Link>
        </div>
